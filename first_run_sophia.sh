...
if [[ -v CNSIM_FILE ]]; then
    ...
    opal import-csv --o https://localhost:8443 --u administrator --p $OPAL_ADMINISTRATOR_PASSWORD --destination test --path /home/administrator/$CNSIM_FILE --tables CNSIM --separator , --type CNSIM
    opal perm-table --o https://localhost:8443 --u administrator --p $OPAL_ADMINISTRATOR_PASSWORD --type USER --project test --subject guest  --permission view --add --tables CNSIM
fi
